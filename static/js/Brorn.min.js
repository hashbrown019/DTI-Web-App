...
function $show_view_args_check(ids,classn, istrue,isfalse){
	...
	println(ids)
	for (var i = 0; i < panel_views.length; i++) {
		if(ids!=panel_views[i].id){panel_views[i].checked=istrue}
		else{panel_views[i].checked=isfalse}
	}
 }
...
function $dialog(args){
	var ___name = create_div()
	var MODAL_ = $ID(___name)
	// var MODAL_ = create_div()
	MODAL_.style.display ="none"
	var contnt = $dialog_popu(args,MODAL_)
	var type_ls__ = {"success":"green","error":"red","warning":"yellow","neutral":"grey"}
	var type_ls__ICON = {"success":"check-square-o","error":"minus-circle","warning":"exclamation-triangle","neutral":"info-circle"}
	...
	var __btns = contnt.buttons
	var __btns_def = MODAL_.querySelectorAll(".___modal_btn")
	var __btns_act = contnt.buttons_actions
	...
	for (var _btnACT = 0; _btnACT < __btns_act.length; _btnACT++) {
		__btns_def[_btnACT].addEventListener("click",__btns_act[_btnACT])
	}

	var CLR_ = type_ls__[contnt.type]
	var CLR_ICON = type_ls__ICON[contnt.type]
	...
	var __value_setter = function(){

	}
	return {
		"get_modal_data" : contnt,
		"dialog": MODAL_.childNodes[0],
		"show": function(){MODAL_.style.display="block"},
		"hide":function(){ MODAL_.style.display="none"},
		"destroy":function(){ DESTROY_MODAL(MODAL_)},
		"elements": MODAL_.childNodes,
		"modal": MODAL_,
	}
}

function DESTROY_MODAL(__MODAL){
	__MODAL.remove();
}
...
const MODAL_DEF = `<div id="MODAL_BODYBRN" class="x-modal-content x-animate-top"><div id="MODAL_BRN_HEADER" class="x-container x-padding"><span onclick="this.parentNode.parentNode.parentNode.style.display='none';DESTROY_MODAL(this.parentNode.parentNode.parentNode)" class="x-large x-hover-red x-button x-display-topright fa fa-times-rectangle-o x-text-red"></span><div style=""><span><span class="x-xlarge"><b id="modal_title">Title</b><br><span class="x-small" id="modal_subtitle">Sub title</span></span><br></span></div></div><div class="x-container"><span></span><span id="modal_content">Content</span></div><div class="x-container x-padding x-light-grey"><span id="FA_FA_" class="x-left x-xxlarge"></span><div id="modal_footer_content" class="x-left x-text-grey x-padding x-display-container">Footer Content</div><div class="x-right"><button id="___modal_btn1" style="display:none;" class="___modal_btn x-btn x-green">Button1</button><button id="___modal_btn2" style="display:none;" class="___modal_btn x-btn x-red">Button2</button><button id="___modal_btn3" style="display:none;" class="___modal_btn x-btn x-orange">Button3</button></div></div></div>`;
...
function $dropdown_multiple(params){
	...
	var _opts = (params.search) ?   "<div class='x-row'><input onkeyup='__dd_on_search(this)' style='width:70%;' class='__dd_search_field x-col x-input x-border' type='' name='' placeholder='Search or add here'>"+btn__dd_ad+"</div><b class='x-text-grey'>Selected</b><div class='__dd_added_item'></div><br><b class='x-text-grey'>Options</b>":""
	...
	for (var i = 0; i < params.option.length; i++) {
		_opts +=(`<li style="padding:2px;" class="__item_ind x-hover-blue x-text-grey"><span class='_dd_item_label'>`+params.option[i]+`</span></li>`)
	}
	__id_dd_m.getElementsByClassName('anchor')[0].innerHTML = params.default;
	...
	__id_dd_m.getElementsByClassName('anchor')[0].onclick = function(evt) {
		...
		else{__id_dd_m.classList.add('visible');}1
	}
	...
	return {
		...
		add_item: function(itm_val){
			if(dd_value(__id_dd_m).includes(itm_val)){println(" * Item on parameter failed to add due to dupilciate");return}
...
function added_item_click_dd(elem){
	...
	parent_node.getElementsByClassName('anchor')[0].innerHTML = "<b>("+dd_value(parent_node).length + ")</b> Selected";
}
